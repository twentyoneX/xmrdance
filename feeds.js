...
function get_marketplaces() {
  return [
    ...
    // 2. RSS2JSON ROUTE (ONLY for Monero Talk & Moon which block proxies)
    ...
    // 4. STANDARD RSS/ATOM (Fetched via Proxy Fallback)
...
async function fetch_data(url, format) {
  // Strategy A: RSS2JSON (Strictly for blocked feeds)
  if (format === 'rss2json') {
    try {
  ...
  // Switched priority: AllOrigins is more reliable for JSON APIs and some XML than Corsproxy currently
  const proxies = [
        ...
        if (text && !text.includes('Access Denied') && !text.includes('Cloudflare') && !text.includes('403 Forbidden')) {
          ...
          if (format === 'api_json') {
             try {
                ...
                if (json.contents && typeof json.contents === 'string') { // Unwrap AllOrigins
